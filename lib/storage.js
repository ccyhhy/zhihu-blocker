...
const ZBStorage = (() => {
  ...
  const DEFAULT_SETTINGS = {
    ...
    blockConcurrency: 5,      // 0 秒间隔时的并发拉黑数
  ...
  async function getBlockState() {
    ...
    const [settings, data] = await Promise.all([
      getSettings(),
      get({ blockedUsers: {}, [key]: 0 }),
    ]);
    return {
      settings,
      dailyKey: key,
      dailyCount: data[key] || 0,
  ...
  async function saveBlockState(state) {
    await set({
      ...
      [state.dailyKey]: state.dailyCount || 0,
    });
  }
  ...
  return {
    ...
    getBlockedSet, isBlocked, markBlocked, getBlockState, saveBlockState,
